...
def api():
    ...
    if check_rate_limit(id):
        ...
        if len(errors) > 0:
            ...
            response = g4f.ChatCompletion.create(model='gpt-3.5-turbo', provider=g4f.Provider.g4f.Provider.Theb, messages=messages)
...
def transcript():
    ...
    proxy=os.getenv('PROXY2'),
    thingtosearch = g4f.ChatCompletion.create(model=g4f.models.default, provider=g4f.Provider.AItianhuSpace, messages=messages1)
    if internet == "on":
        async def search2():
    ...
    proxy=os.getenv('PROXY2'),
    response = g4f.ChatCompletion.create(model='gpt-3.5-turbo', provider=g4f.Provider.Theb, messages=messages)
    return flask.make_response(response), 200
    run(search2())
