  ...
  if check_rate_limit(id):
      ...
      if len(err) > 0:
          ...
          if internet == "on":
              ...
              proxy=os.getenv('PROXY2'),
              finalresponse = g4f.ChatCompletion.create(model=g4f.models.default, provider=g4f.Provider.ChatgptAi, messages=messages) #, cookies={"token": os.getenv('HFCOOKIE')}, auth=True)
